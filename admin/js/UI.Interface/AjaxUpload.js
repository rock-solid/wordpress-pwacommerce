...
function PWACAjaxUpload() {
  ...
  this.frame = function(c) {
    ...
    jQuery('body *:first', window.document).before(
      '<div><iframe style="display:none" src="about:blank" id="' +
        n +
        '" name="' +
        n +
        '"></iframe></div>'
  ...
  this.loaded = function(id) {
    ...
    if (
      typeof jQuery('#' + id, window.document).get(0).onComplete == 'function'
    ) {
      jQuery('#' + id, window.document)
        .get(0)
