...
function PWACPreloader() {
  ...
  this.start = function(params) {
    ...
    jQuery('body *:first', document).before(
      '<div id="preloader_container" style="position:fixed; z-index:999998; display:none;"><div class="preloader"></div><div style="position:fixed; background: #000;"><div id="preloader_table" align="center" style="font-family:Arial, Helvetica, sans-serif; font-size:12px; color:#FFF; padding:10px;">' +
        this.defaultParams.message +
        '<br><br><img src="' +
        PWACJSInterface.localpath +
    ...
    preloader_container
      .css({ top: '50%', left: '50%', 'margin-left': newW, 'margin-top': newH })
      .fadeIn(500);
...
function PWACMessage() {
  ...
  this.view = function(params) {
    ...
    var no_messages = jQuery(
      'div[id="pwacMessageBox"]',
      this.parentContainer
    ).get().length;

    // add message box to the parent container
    var html = '<div id="pwacMessageBox" class="loader" style="display:none;">';
    html += '<div id="pwacBoxTable" align="left" style="background: #fefcd4;">';
    html +=
      '<div style="float:right; height:15px; padding-right:5px; padding-top:5px"><img id="pwacCloseMsg" src="' +
      PWACJSInterface.localpath +
      'admin/images/btn_close_msg.png" border="0" style="cursor:pointer" /></div>';
    html +=
      '<div align="center" style="font-family:Arial, Helvetica, sans-serif; font-size:12px; color:#000; padding:15px;">' +
      this.defaultParams.message +
      '</div>';
    html += '</div>';
    html += '</div>';
    ...
    this.container = jQuery(
      'div[id="pwacMessageBox"]:eq(' + no_messages + ')',
      this.parentContainer
    );
    var table = jQuery('#pwacBoxTable', this.container);
    var close_btn = jQuery('#pwacCloseMsg', table);
    ...
    this.container
      ...
      .slideDown(this.defaultParams.speed);
    ...
    this.container.bind('countTimer', function() {
      ...
      if (seconds / 1000 - 1 >= 1) {
        ...
        if (typeof JSObject.defaultParams.closeFunction == 'function')
          JSObject.defaultParams.closeFunction();
      }
    });
...
function PWACLoader() {
  ...
  /**
     ...
     */
  this.init = function() {
    jQuery('#pwac_messages_container').remove();
    jQuery(jQuery('body').get(0)).append(
      '<div align="center" id="pwac_messages_container" style="position:fixed; z-index:999999; width: 100%; display:block; top: 5px; margin: 0 auto;"></div>'
    );

    var messages_container = jQuery('#pwac_messages_container');
  };
  ...
  this.display = function(params) {
    ...
    var newMessage = new PWACMessage();
